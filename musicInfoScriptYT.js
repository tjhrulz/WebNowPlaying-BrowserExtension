...
var onMessage = function(event) {
	try {
		if (document.getElementsByClassName("ytd-video-primary-info-renderer title").length > 0) {
			...
			//Make go to beginning of video if not in playlist
				...
				if (event.data.toLowerCase() == "repeat") {
					document.getElementsByClassName("yt-playlist-buttons")[0].children[0].children[0].click();
				}
				else if (event.data.toLowerCase() == "shuffle") {
					document.getElementsByClassName("yt-playlist-buttons")[0].children[0].children[1].click();
				}
			}
		...
		//Old youtube
		else if (document.getElementsByClassName("watch-title").length > 0) {
			if (event.data.toLowerCase() == "playpause") {
				document.getElementsByClassName("ytp-play-button")[0].click();
			}
			else if (event.data.toLowerCase() == "next") {
			...
			else if (document.getElementsByClassName("playlist-nav-controls").length > 0 && (event.data.toLowerCase() == "repeat" || event.data.toLowerCase() == "shuffle")) {
				if (event.data.toLowerCase() == "repeat") {
			...
			//Old youtube has diferennt buttons for undoing thumbs up and down
			else if (event.data.toLowerCase() == "togglethumbsup") {
				...
				//If currently thumbed up undo it
				if(thumbsUp)
				{
				...
				//If currently thumbed down undo it
				if(thumbsDown)
				{
	...
	try {
		if (window.location.href.indexOf("v=") > 0) {
			...
			//Old youtube
				...
				var newTitle = document.getElementsByClassName("watch-title")[0].innerText;
				if (newTitle != oldTitle) {
					oldTitle = newTitle;
					ws.send("TITLE:" + newTitle);
				}
				...
				var newArtist = document.getElementsByClassName("yt-user-info")[0].children[0].innerText;
				if (newArtist != oldArtist) {
					oldArtist = newArtist;
					ws.send("ARTIST:" + newArtist);
				}

				if (document.getElementById("player-playlist").children.length > 0) {
					var newAlbum = document.getElementsByClassName("playlist-title")[0].innerText;
					if (newAlbum != oldAlbum) {
						oldAlbum = newAlbum;
						ws.send("ALBUM:" + newAlbum);
					}

					var newRepeat = document.getElementsByClassName("playlist-nav-controls")[0].children[0].getAttribute("class").includes("toggled");
					if (newRepeat != oldRepeat) {
						oldRepeat = newRepeat;
						var repeat = 0;
					...
					var newShuffle = document.getElementsByClassName("playlist-nav-controls")[0].children[1].getAttribute("class").includes("toggled");
					if (newShuffle != oldShuffle) {
						oldShuffle = newShuffle;
						var Shuffle = 0;
